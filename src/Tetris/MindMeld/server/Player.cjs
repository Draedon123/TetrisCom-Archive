...
class Player extends EventEmitter {
    ...
    switch (message.type) {
      ...
      case "lockPiece": {
        ...
        for (const player of this.room.players) {
          ...
          player.sendLockPiece(message.lockNumber);
        }
   ...
   */
  sendLockPiece(lockNumber) {
    /** @type { import("./messageTypedefs.cjs").LockPieceMessage } */
    const message = { type: "lockPiece", lockNumber };

    this.client.sendText(JSON.stringify(message));
  }

  sendHoldPiece() {
    /** @type { import("./messageTypedefs.cjs").HoldPieceMessage} */
